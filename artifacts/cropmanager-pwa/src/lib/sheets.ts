...
export const mappers = {
  crop: (r: any[]): Crop => ({
    ...
    nextConsistentPlanting: r[11] ?? '', 
    batchNumber: Number(r[12] || 0), 
    fungusSprayDates: r[13] ?? '', 
    pestSprayDates: r[14] ?? '', 
    status: r[15] ?? 'Active', 
    notes: r[16] ?? '', 
    daysSeedGerm: Number(r[17] || 0), 
    daysGermTransplant: Number(r[18] || 0), 
    daysTransplantHarvest: Number(r[19] || 0), 
    telegramChatId: r[20] ?? '', 
    syncStatus: 'clean', 
    updatedAt: Date.now()
  }),
