...
export const mappers = {
  crop: (r: any[]): Crop => ({
    ...
    harvestFrequency: Number(r[12] || 0) || undefined,
    numPlots: Number(r[13] || 0) || undefined,
    batchOffset: Number(r[14] || 0) || undefined,
    nextConsistentPlanting: r[15] ?? '', 
    batchNumber: Number(r[16] || 1), 
    fungusSprayDates: r[17] ?? '', 
    pestSprayDates: r[18] ?? '', 
    fertilizerType: r[19] ?? '',
    fertilizerDays: Number(r[20] || 0),
    nextFertilizerDate: r[21] ?? '',
    status: r[22] ?? 'Active', 
    notes: r[23] ?? '', 
    daysSeedGerm: Number(r[24] || 0), 
