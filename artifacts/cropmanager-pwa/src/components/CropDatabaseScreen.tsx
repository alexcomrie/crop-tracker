...
import { Search, Plus, Trash2, Download, ChevronLeft, X, ArrowRight, Tag } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-white animate-in slide-in-from-right duration-300">
      <header className="flex items-center justify-between p-4 border-b">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-bold text-lg">Crop Database</h2>
        </div>
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={handleSaveDb} className="text-green-700">
             Save File
          </Button>
          <Button variant="ghost" size="sm" onClick={exportJson} className="text-green-700">
        ...
        <div className="w-full md:w-64 border-r flex flex-col h-full">
          <div className="p-3 border-b space-y-2">
            ...
            <button 
              ...
              className={`text-xs font-semibold px-2 py-1 rounded border transition-colors ${showAliases ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-gray-50 border-gray-200 text-gray-500'}`}
            >
              {showAliases ? 'Showing Aliases' : 'Show Aliases'}
            </button>
          </div>
        ...
        <div className="hidden md:block flex-1 overflow-y-auto p-6 bg-gray-50">
          {!selectedKey ? (
            ...
            <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl border shadow-sm space-y-6">
               <h2 className="text-2xl font-bold">{selectedKey}</h2>
               <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl flex items-center gap-4">
                 <div className="text-2xl">🔗</div>
                 <div className="flex-1">
                   <p className="font-bold text-amber-900">This is an alias entry</p>
                   <p className="text-sm text-amber-700">It redirects to <strong>{selectedEntry.alias}</strong></p>
                 </div>
                 <Button size="sm" onClick={() => setSelectedKey(selectedEntry.alias)} className="bg-amber-600">Go to Target</Button>
               </div>
               <Button variant="outline" className="text-red-600 border-red-200 hover:bg-red-50" onClick={() => handleDelete(selectedKey!)}>
                ...
                return (
                  <>
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="outline" size="sm" className="text-red-600 border-red-200 hover:bg-red-50" onClick={() => handleDelete(selectedKey!)}>
                    ...
                    <section className="bg-white rounded-xl border p-4 space-y-4 shadow-sm">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Display Name</label>
                          <Input 
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Plant Type</label>
                          <select 
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Planting Method</label>
                          <Input 
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[11px] font-bold text-gray-500 uppercase">Varieties (comma separated)</label>
                        <Input 
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                          {entry.varieties.map(v => (
                            <Badge key={v} variant="secondary" className="bg-green-50 text-green-700 border-none font-medium px-2 py-0.5">
                              {v}
                            </Badge>
                          ))}
                        </div>
                    ...
                    {/* Timing */}
                    <section className="bg-white rounded-xl border p-4 space-y-4 shadow-sm">
                      <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Timing & Harvest</h3>
                      <div className="grid grid-cols-3 gap-4">
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Grow Days</label>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Harvest Weeks</label>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Harvest Interval (d)</label>
                          <Input type="number" value={entry.harvest_interval} onChange={e => handleUpdate({ ...entry, harvest_interval: parseInt(e.target.value) || 0 })} />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Germ Min</label>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Germ Max</label>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Batch Offset</label>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Transplant (d)</label>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[11px] font-bold text-gray-500 uppercase">Grow from TP (d)</label>
                      ...
                      <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                        <div>
                          <p className="text-sm font-bold">Continuous Production</p>
                          <p className="text-[10px] text-gray-500">Auto-sets harvest window to 4 weeks</p>
                        </div>
                        <input 
                          type="checkbox" 
                    ...
                    {/* Spray Schedule */}
                    <section className="bg-white rounded-xl border p-4 space-y-4 shadow-sm">
                      <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Spray Schedule (Days)</h3>
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <label className="text-[11px] font-bold text-red-600 uppercase">Fungus Spray Days</label>
                          <div className="flex flex-wrap gap-2">
                            {entry.fungus_spray_days.map((d, i) => (
                              <Badge key={i} variant="secondary" className="bg-red-50 text-red-700 border-none px-2 py-1 flex items-center gap-1">
                                Day {d}
                                <button onClick={() => {
                        ...
                        <div className="space-y-2">
                          <label className="text-[11px] font-bold text-amber-600 uppercase">Pest Spray Days</label>
                          <div className="flex flex-wrap gap-2">
                            {entry.pest_spray_days.map((d, i) => (
                              <Badge key={i} variant="secondary" className="bg-amber-50 text-amber-700 border-none px-2 py-1 flex items-center gap-1">
                                Day {d}
                                <button onClick={() => {
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <section className="bg-white rounded-xl border p-4 space-y-3 shadow-sm">
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Diseases</h3>
                        <div className="flex flex-wrap gap-1">
                          {entry.diseases.map((d, i) => (
                            <Badge key={i} className="bg-gray-100 text-gray-700 border-none">
                              {d} <button onClick={() => {
                        ...
                        <Input placeholder="Add disease..." onKeyDown={e => {
                      ...
                      <section className="bg-white rounded-xl border p-4 space-y-3 shadow-sm">
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Pests</h3>
                        <div className="flex flex-wrap gap-1">
                          {entry.pests.map((p, i) => (
                            <Badge key={i} className="bg-gray-100 text-gray-700 border-none">
                              {p} <button onClick={() => {
                        ...
                        <Input placeholder="Add pest..." onKeyDown={e => {
                          if (e.key === 'Enter' || e.key === ',') {
                            e.preventDefault();
                            const val = e.currentTarget.value.trim();
                            if (val) {
